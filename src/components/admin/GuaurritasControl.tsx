...
export default function GuaurritasControl() {
  ...
  <div className="mt-4 grid gap-4 xl:grid-cols-2"><section className="min-w-0 overflow-hidden rounded-2xl border border-[#e0e4f0] bg-white p-4 shadow-sm"><h2 className="font-title text-xl text-[#25418e]">Top productos</h2><p className="font-interface text-xs text-slate-400">Por ventas pagadas en el periodo</p><div className="mt-4 space-y-4">{(dashboard?.topProducts || []).map((product) => <div key={product.name} className="min-w-0 overflow-hidden"><div className="break-words font-interface text-xs font-semibold leading-4 text-slate-600">{product.name}</div><div className="mt-1 flex min-w-0 flex-wrap items-center justify-between gap-x-3 gap-y-1 font-interface text-[10px]"><span className="font-bold text-[#425BBC]">Ventas {money(product.sales)}</span><span className="shrink-0 text-slate-400">{product.units} {product.units === 1 ? 'unidad' : 'unidades'}</span></div><div className="mt-1.5 h-1.5 overflow-hidden rounded-full bg-[#edf0f7]"><div className="h-full rounded-full bg-[#4d67c9]" style={{ width: `${Math.max(5, (product.sales / maxProductSales) * 100)}%` }} /></div></div>)}</div></section><section className="rounded-2xl border border-[#e0e4f0] bg-white p-4 shadow-sm"><h2 className="font-title text-xl text-[#25418e]">Últimos pedidos</h2><p className="font-interface text-xs text-slate-400">Actividad más reciente</p><div className="mt-4 grid gap-2">{(dashboard?.recentOrders || []).slice(0, 5).map((order) => <OrderCard key={order.id} order={order} onOpen={openDetail} />)}</div></section></div></> : null}
